...
class Agent():
    ...
    def save_model(self, chkpt):
    	print('--------Saving Model--------')
    	torch.save(self.actor.state_dict(), chkpt)
    	torch.save(self.actor_target.state_dict(), chkpt)
    	torch.save(self.critic.state_dict(), chkpt)
    	torch.save(self.critic_target.state_dict(), chkpt)
    	
    def load_model(self, chkpt):
    	print('--------Loading Model--------')
    	self.actor.load_state_dict(torch.load(chkpt))
    	self.actor_target.load_state_dict(torch.load(chkpt))
    	self.critic.load_state_dict(torch.load(chkpt))
    	self.critic_target.load_state_dict(torch.load(chkpt))
